...
class ApiService {
    ...
    try {
      ...
      if (response.body.contains('<!DOCTYPE html>') ||
        ...
        return {
          'status': false,
          'message':
              'Server Error (HTML): Gagal terhubung ke API. Pastikan file index.php dan .htaccess sudah diupload dengan benar di Hosting.',
        };
      }
    ...
    try {
      ...
      if (response.body.contains('<!DOCTYPE html>') ||
        ...
        return {
          'status': false,
          'message':
              'Server memberikan respons HTML (Error). Mohon cek konfigurasi Hosting.',
        };
      }
  ...
  static Future<Map<String, dynamic>> deleteBooking(
    ...
    return post(
      "/delete_data",
      data: {"type": "booking", "id": bookingId, "token": token},
    );
  }

  static Future<Map<String, dynamic>> getFoodMenu() async {
    return await get("/get_data?table=food_menu");
  }
  ...
  static Future<Map<String, dynamic>> checkApiStatus() async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/test_conn.php'))
